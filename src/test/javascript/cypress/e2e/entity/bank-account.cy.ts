...
describe('BankAccount e2e test', () => {
  ...
  let username: string;
  let password: string;
  const bankAccountSample = { name: 'stormy phew', balance: 21800.87 };
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/jhipstersamplemicroservice/api/bank-accounts+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/jhipstersamplemicroservice/api/bank-accounts').as('postEntityRequest');
    cy.intercept('DELETE', '/services/jhipstersamplemicroservice/api/bank-accounts/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (bankAccount) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/jhipstersamplemicroservice/api/bank-accounts/${bankAccount.id}`,
      }).then(() => {
  ...
  describe('BankAccount page', () => {
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/jhipstersamplemicroservice/api/bank-accounts',
  ...
  describe('new BankAccount page', () => {
    ...
    it('should create an instance of BankAccount', () => {
      cy.get(`[data-cy="name"]`).type('because');
      cy.get(`[data-cy="name"]`).should('have.value', 'because');

      cy.get(`[data-cy="balance"]`).type('9568.17');
      cy.get(`[data-cy="balance"]`).should('have.value', '9568.17');
